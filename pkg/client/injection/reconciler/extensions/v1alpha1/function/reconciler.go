...
import (
	...
	extensionsv1alpha1 "github.com/hyperfunction/hyperfunction/pkg/client/listers/extensions/v1alpha1"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha1.Function.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Function. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Function) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Function.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Function. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Function) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.Function if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Function.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Function) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.Function) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.Function resources.
type reconcilerImpl struct {
	...
	Lister extensionsv1alpha1.FunctionLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister extensionsv1alpha1.FunctionLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Functions(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.Function, desired *v1alpha1.Function) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ServingV1alpha1().Functions(desired.Namespace)
		...
		updater := r.Client.ServingV1alpha1().Functions(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Function) (*v1alpha1.Function, error) {

	getter := r.Lister.Functions(resource.Namespace)
	...
	patcher := r.Client.ServingV1alpha1().Functions(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Function) (*v1alpha1.Function, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Function, reconcileEvent reconciler.Event) (*v1alpha1.Function, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
